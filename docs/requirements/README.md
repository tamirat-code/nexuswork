...
| FR-28 | Portable reputation record with export, verification, and lifecycle status | ✅ | `GET /v1/reviews/user/:userId/reputation/export` builds a versioned, privacy-safe signed `NexusWorkReputationCredential` from reviews, delivery milestones, dispute aggregates, verified skills, and credential references; `POST /v1/reviews/reputation/verify` validates the signature and rejects revoked/superseded exports; admins can update lifecycle status through `PATCH /v1/reviews/user/:userId/reputation/status`. The document is NexusWork-versioned; formal external credential-standard conformance remains a separate FR-19 gate |
