...
const AccordionItem = React.forwardRef(function AccordionItem({ className, ...props }, ref) {
  return <AccordionPrimitive.Item ref={ref} className={cn("border-b border-ink-300", className)} {...props} />;
});
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(
          "flex flex-1 items-center justify-between py-4 text-sm font-semibold text-slate transition-all hover:text-brass [&[data-state=open]>svg]:rotate-180",
          className
        )}
        {...props}
      >
