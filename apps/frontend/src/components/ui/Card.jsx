  ...
  return (
    <Tag
      className={cn(
        ...
        padded && "p-5 sm:p-6",
        interactive && "cursor-pointer transition-all duration-200 hover:border-brass/40 hover:shadow-elevated",
        className
      )}
      {...props}
    >
  ...
  return (
    <div className={cn("flex flex-wrap items-start justify-between gap-3", className)}>
      <div className="min-w-0">
        <TitleTag className="font-display text-lg font-semibold leading-tight text-slate">{title}</TitleTag>
        {description && <p className="mt-1.5 text-sm leading-relaxed text-slate-300">{description}</p>}
      </div>
