/**
 * Strands.jsx — animated WebGL background effect
 * Single-file version: component + styles, nothing else to import.
 *
 * ── HOW TO USE ──────────────────────────────────────────────────────────
 * 1. Install the one dependency:
 *      npm install ogl
 *
 * 2. Drop this file into your project (e.g. src/components/Strands.jsx).
 *
 * 3. Import and use it as an absolutely-positioned background behind your
 *    hero content:
 *
 ...
 *              <p>Sits on top of the animation because of the higher z-index.</p>
 *            </div>
 *          </div>
 *        );
 *      }
 *
 *    The outer wrapper needs `position: relative` and a real height
 *    (min-height, height, or flex-grow) — the canvas fills its parent.
 *
 * 4. Customize by passing props, e.g.:
 *      <Strands colors={['#ff4242', '#7c3aed', '#06b6d4']} count={5} speed={0.6} />
 *
 *    Key props:
 *      colors      array of hex strings — the strand palette
 *      count       number of strands, 1–12 (default 4)
 *      speed       animation speed (default 0.4)
 *      glow        glow intensity (default 2.2)
 *      opacity     overall opacity 0–1 (default 0.85)
 *      scale       zoom level (default 1.6)
 *      glass       true/false — adds a lens/glass refraction overlay (default false)
 * ───────────────────────────────────────────────────────────────────────
 */
...
export default function Strands({
  colors = ['#00c8b4', '#0e7fa3', '#7C3AED'], // CollabFlow palette: teal, blue, purple
  count = 4,
  speed = 0.4,
  amplitude = 1,
  waviness = 1,
  ...
  return (
    <div ref={ctnDom} className={`strands-container ${className}`} style={style}>
      {/* Styles are inlined here so this is a single, drop-in file */}
      <style>{`
        .strands-container {
          position: relative;
          width: 100%;
