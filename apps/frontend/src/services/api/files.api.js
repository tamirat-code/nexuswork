...
// related_type: one of the File model's enum values, e.g. "verification_document".
export const uploadFile = (file, { relatedType, relatedId, token } = {}) => {
  ...
  return apiRequest("/files/upload", { method: "POST", body: form, token });
};

export const getFile = (id, token) => apiRequest(`/files/${id}`, { token });

export const deleteFile = (id, token) => apiRequest(`/files/${id}`, { method: "DELETE", token });