  ...
  return (
    <div className="mx-auto max-w-5xl animate-fade-up">
      <header className="border-b border-ink-300 pb-6">
        ...
        <h1 className="mt-2 font-display text-3xl tracking-tight text-slate">Payments</h1>
        <p className="mt-2 text-sm text-slate-300">Every escrow transaction, funded milestone, and release.</p>
      </header>
          ...
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading && payments.length === 0 && (
                <TableRow><TableCell colSpan={5} className="py-14 text-center text-slate-300">No payments yet. Funded milestones appear here.</TableCell></TableRow>
              )}
              {payments.map((p) => (
                <TableRow key={p._id}>
                  <TableCell className="font-mono text-sm text-brass">#{p.reference || p._id.slice(-6)}</TableCell>
                  <TableCell className="text-sm text-slate-300">{p.milestone_id?.title || "Milestone"}</TableCell>
