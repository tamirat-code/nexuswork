...
function MilestoneCard({ milestone, role, onAction }) {
      ...
      <CardContent className="p-4 sm:p-5">
        ...
        {(canFund || canDeliver || canApprove || canDispute) && (
          <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-ink-300 pt-3">
            {canFund && (
              <Button size="sm" onClick={() => onAction("fund", milestone)}>
                Fund milestone
              </Button>
            )}
            {canDeliver && (
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto py-4 pr-1" aria-live="polite">
        ...
        {messages.map((m) => (
          <div
            key={m._id}
            className={`flex ${m.sender_id?._id === contract?.student_id || m.sender_id?._id === contract?.student_id
              ? "justify-start" : "justify-start"}`}
          >
            <div className="max-w-[85%] rounded-card border border-ink-300 bg-ink-700 px-3.5 py-2.5">
              <p className="text-xs font-semibold text-brass">{m.sender_id?.name || "User"}</p>
      ...
      <form
        ...
        onSubmit={(e) => {
          ...
          // Optimistic send through the REST API; Socket.io picks this up in real apps.
          toast.success("Message sent");
          setMessage("");
        }}
      >
        <Button type="button" variant="secondary" size="icon" className="h-11 w-11 shrink-0" aria-label="Attach file">
...
export default function ContractDetailPage() {
  ...
  const fundMutation = useMutation({
    ...
    onError: (err) => toast.error(err.message || "Could not fund milestone"),
  });
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex flex-wrap items-center gap-2">
                <Button variant="secondary" size="sm"><Paperclip className="h-4 w-4" /> Upload files</Button>
        ...
        <aside className="space-y-6 lg:sticky lg:top-24">
          ...
          <Card>
            <CardContent className="h-[420px] p-4">
              <ChatPanel contractId={id} token={token} contract={contract} />
            </CardContent>
          </Card>
        </aside>
      </div>
