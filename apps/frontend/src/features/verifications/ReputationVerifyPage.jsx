      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        ...
        <Card as="section" className="p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="font-display text-xl font-bold text-content-primary">{t("reputationVerification.inputTitle")}</h2>
              <p className="mt-1 text-sm text-content-secondary">{t("reputationVerification.inputDescription")}</p>
            </div>
          ...
          <p className="mt-2 text-xs text-content-muted">{t("reputationVerification.allowed")}</p>
          ...
          {result && (
            <div className="space-y-5">
              ...
              <div className="grid gap-3 rounded-card border border-border-subtle bg-surface-soft p-4 text-sm sm:grid-cols-2">
                <div><p className="text-xs font-extrabold uppercase tracking-widest text-content-secondary">{t("reputationVerification.subject")}</p><p className="mt-1 break-all font-bold text-content-primary">{result.subject || "Not provided"}</p></div>
                <div><p className="text-xs font-extrabold uppercase tracking-widest text-content-secondary">{t("reputationVerification.issuer")}</p><p className="mt-1 font-bold text-content-primary">{result.issuer || "Not provided"}</p></div>
                <div><p className="text-xs font-extrabold uppercase tracking-widest text-content-secondary">{t("reputationVerification.issued")}</p><p className="mt-1 font-bold text-content-primary">{formatDate(result.issuedAt)}</p></div>
                <div><p className="text-xs font-extrabold uppercase tracking-widest text-content-secondary">{t("reputationVerification.version")}</p><p className="mt-1 font-bold text-content-primary">{result.version || "Not provided"}</p></div>
              </div>
              ...
              {!result.valid && (
                <div className="flex gap-3 rounded-card border border-danger/30 bg-danger-soft p-4 text-sm text-content-primary">
...
function Metric({ icon: Icon, label, value }) {
  return (
    <div className="rounded-card border border-border-subtle bg-surface-soft p-4">
      ...
      <p className="mt-3 text-2xl font-black text-content-primary">{value}</p>
      <p className="mt-1 text-xs font-semibold text-content-secondary">{label}</p>
    </div>
  );
}
