    ...
    <div className="w-full animate-fade-up space-y-6">
      <header className="relative overflow-hidden rounded-card border border-ink-300 bg-ink-900 px-6 py-8 shadow-card sm:px-8">
        ...
        <div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div className="max-w-2xl"><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-brass">{t("organizations.workspace", { defaultValue: "Organization workspace" })}</p><h1 className="mt-2 font-display text-3xl tracking-tight text-slate sm:text-4xl">{t("organizations.title", { defaultValue: "Your operating teams" })}</h1><p className="mt-3 text-sm leading-6 text-slate-300">{t("organizations.description", { defaultValue: "Centralize members, billing context, and institution relationships across every client workspace you manage." })}</p></div>
          <div className="flex items-center gap-2 rounded-control border border-escrow/30 bg-escrow/10 px-3 py-2 text-xs font-medium text-escrow"><CheckCircle2 className="h-4 w-4" aria-hidden="true" />{t("organizations.servicesOperational", { defaultValue: "Workspace services operational" })}</div>
        </div>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(280px,0.7fr)_minmax(0,1.5fr)]">
        <div className="space-y-6">
          <Card>
            <CardHeader className="border-b border-ink-300"><CardTitle>{t("organizations.yourOrganizations", { defaultValue: "Your organizations" })}</CardTitle><CardDescription className="mt-1">{t("organizations.selectManage", { defaultValue: "Select a workspace to manage access and members." })}</CardDescription></CardHeader>
            <CardContent className="space-y-2 pt-5">
              ...
              {organizations.isError && <p className="rounded-control border border-brick/30 bg-brick/10 p-3 text-sm text-brick">{t("organizations.unableToLoad", { defaultValue: "Unable to load your organizations. Refresh and try again." })}</p>}
              {!organizations.isLoading && !organizations.isError && orgs.length === 0 && <div className="rounded-control border border-dashed border-ink-300 p-4 text-sm text-slate-300">{t("organizations.emptyState", { defaultValue: "You do not belong to an organization yet. Create your first workspace below." })}</div>}
              {orgs.map((org) => <button type="button" key={org._id} onClick={() => setSelectedId(org._id)} className={`w-full rounded-control border p-4 text-left transition-colors ${String(org._id) === String(organizationId) ? "border-brass bg-brass/10 shadow-sm" : "border-ink-300 bg-ink-50 hover:border-brass/50"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold text-slate">{org.name}</p><p className="mt-1 text-xs text-slate-300">{org.institution_id?.name || t("organizations.independentWorkspace", { defaultValue: "Independent workspace" })}</p></div><Badge variant={String(org._id) === String(organizationId) ? "outline" : "secondary"}>{roleLabels[org.membership?.role] || org.membership?.role}</Badge></div></button>)}
            </CardContent>
          </Card>

          {(user?.role === "client" || user?.role === "admin") && <Card className="border-brass/25">
            <CardHeader><div className="flex items-center gap-3"><div className="rounded-control bg-brass/10 p-2 text-brass"><Plus className="h-5 w-5" aria-hidden="true" /></div><div><CardTitle>{t("organizations.createWorkspace", { defaultValue: "Create workspace" })}</CardTitle><CardDescription className="mt-1">{t("organizations.createDescription", { defaultValue: "Set up a new client operating environment." })}</CardDescription></div></div></CardHeader>
