...
export default function OrganizationsPage() {
  ...
  const saveSettings = useMutation({
    mutationFn: () => updateOrganization(organizationId, {
      name: settingsName.trim(),
      billing_mode: settingsBillingMode,
      institution_id: settingsInstitutionId === "none" ? null : settingsInstitutionId,
    }, token),
    onSuccess: ({ data }) => {
            ...
            <CardContent className="space-y-6 pt-6">
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(260px,0.9fr)]">
                <div className="rounded-card border border-ink-300 bg-ink-50 p-5">
                  <div className="flex items-center justify-between gap-3"><div><h3 className="font-semibold text-slate">Workspace settings</h3><p className="mt-1 text-xs text-slate-300">Control the organization identity, institution relationship, and billing model.</p></div>{selected.institution_id ? <Badge variant="outline">Institution linked</Badge> : <Badge variant="secondary">Independent</Badge>}</div>
                  {isAdmin ? <div className="mt-4 grid gap-3 md:grid-cols-3">
                    <div><Label htmlFor="settings-name">Name</Label><Input id="settings-name" className="mt-2" value={settingsName} onChange={(event) => setSettingsName(event.target.value)} /></div>
                    <div><Label htmlFor="settings-institution">Institution</Label><Select value={settingsInstitutionId} onValueChange={setSettingsInstitutionId}><SelectTrigger id="settings-institution" className="mt-2"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Independent workspace</SelectItem>{activeInstitutions.map((institution) => <SelectItem key={institution._id} value={institution._id}>{institution.name}</SelectItem>)}</SelectContent></Select></div>
                    <div><Label htmlFor="settings-billing">Billing</Label><Select value={settingsBillingMode} onValueChange={setSettingsBillingMode}><SelectTrigger id="settings-billing" className="mt-2"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="escrow">Escrow billing</SelectItem><SelectItem value="consolidated_invoice">Consolidated invoice</SelectItem></SelectContent></Select></div>
                    <div className="md:col-span-3 flex justify-end"><Button disabled={!settingsChanged || !settingsName.trim() || saveSettings.isPending} loading={saveSettings.isPending} onClick={() => saveSettings.mutate()}>Save settings</Button></div>
                  </div> : <p className="mt-4 rounded-control border border-ink-300 bg-ink-700 p-3 text-sm text-slate-300">Only workspace admins can edit these settings.</p>}
                </div>
