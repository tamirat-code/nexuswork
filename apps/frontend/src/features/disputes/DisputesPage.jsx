  ...
  return (
    <div className="w-full animate-fade-up">
      <header className="border-b border-ink-300 pb-6">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-brass">{t("disputes.eyebrow")}</p>
        <h1 className="mt-2 font-display text-3xl tracking-tight text-slate">{t("disputes.title")}</h1>
        <p className="mt-2 text-sm text-slate-300">{t("disputes.description")}</p>
      </header>
      ...
      {!isLoading && disputes.length === 0 && (
        <Card className="mt-8 p-14 text-center">
          ...
          <h3 className="mt-4 font-display text-lg text-slate">{t("disputes.noDisputesTitle")}</h3>
          <p className="mt-2 text-sm text-slate-300">{t("disputes.noDisputesDesc")}</p>
        </Card>
      )}
            ...
            <CardContent className="p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold text-slate">{d.milestone_id?.title || t("disputes.milestoneDispute")}</p>
                  <p className="mt-1 text-sm text-slate-300">{d.reason || t("disputes.noReasonProvided")}</p>
                </div>
